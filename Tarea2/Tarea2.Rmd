...
knitr::opts_chunk$set(echo = FALSE, erro = TRUE, fig.align = "center", fig.height = 3.5, fig.pos = "H")
...
```{r, eval = FALSE}
data_num <- data %>% select(age, credit_score, debts, income) %>% as.data.frame()

corrplot(cor(data_num), type = "upper" , method = "numb")
```



```{r lm-multi, echo = TRUE}
model <- lm(income ~ ., data = data[, 2:12, ])
```


```{r, echo = TRUE, fig.width=70}
ols_step_forward_aic(lm(income ~., data=data[, 2:12,]))
```

```{r, echo=TRUE, fig.width=80}
